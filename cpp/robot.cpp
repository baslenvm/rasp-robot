...
int transUart()
{
	if (uart0_filestream != -1)
	{
		int count = write(uart0_filestream, (void*)buff_tx, MAX_PAYLOAD_LEN);		//Filestream, bytes to write, number of bytes to write
		if (count < 0)
		{
...
//Because O_NDELAY has been used this will exit if there are no receive bytes waiting (non blocking read), so if you want to hold waiting for input simply put this in a while loop
	...
	if (uart0_filestream != -1)
	{
		int rx_length = read(uart0_filestream, (void*)buff_rx, MAX_PAYLOAD_LEN);		//Filestream, buffer to store in, number of bytes to read (max)
    if(rx_length <= 0)
    {
      return 0;
    }
